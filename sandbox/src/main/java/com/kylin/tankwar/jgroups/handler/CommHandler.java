...
public class CommHandler implements IHandler {
	...
	public void sendHandler(Tank tank, Communication comm, Event event) {
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Tank Status parameters - " + tank.getView());
		}
	...
	public void recieveHandler(Tank tank, Map<String,Tank> tankMap, Session session, MainFrame mainFrame, Event event) {
		...
		for(Entry<String, TankView> entry : session.getTankViewMap().entrySet()) {
			
			if(entry.getKey().compareTo(tank.getId()) == 0) {
				continue;
			}
			
			if(entry.getValue().isLive() == false) {
				deathId = entry.getKey();
			}
			
			if(tankMap.get(entry.getKey()) == null) {
				Tank t = new Tank(entry.getValue());
				tankMap.put(t.getId(), t);
			} else {
				tankMap.get(entry.getKey()).updateTank(session.getTankView(entry.getKey()));
			}
		}
		
		if(null != deathId) {
			tankMap.remove(deathId);
			mainFrame.getComm().getSession().removeTankView(deathId);
		}
